...
class TestSignalHandling:
    ...
    def test_signal_handler_registration(self, mock_signal_handler: Callable[[int, Any], None]) -> None:
    ...
    @pytest.mark.parametrize("signal_number,signal_name", [
    ...
    def test_graceful_shutdown_timeout(self, shutdown_event: threading.Event) -> None:
        ...
        assert elapsed_time >= timeout_duration * 0.9, "Should wait approximately the full timeout duration"
        assert elapsed_time <= timeout_duration * 1.5, "Should not wait too much longer than timeout"
