...
class TestTSVConversionOptions:
    ...
    @pytest.mark.parametrize(
        "case_insensitive,preserve_original_case",
        [
            (True, False),
            (True, True),
            (False, False),
    ...
    def test_custom_options(self, case_insensitive: bool, preserve_original_case: bool) -> None:
        ...
        options = TSVConversionOptions(
            case_insensitive=case_insensitive, preserve_original_case=preserve_original_case
        )
...
class TestCaseSensitiveConversionStrategy:
    ...
    def test_exact_match_conversion(
        ...
        expected = (
            "Use Application Programming Interface and Structured Query Language for development"
        )
    ...
    @pytest.mark.parametrize(
        "test_text",
        ["Use REST and GraphQL", "Use NoMatch and Unknown", "This has no replacements at all"],
    )
...
class TestCaseInsensitiveConversionStrategy:
    ...
    def test_case_insensitive_match(
        ...
        result = strategy.convert_text(
            input_text, default_conversion_dict, case_insensitive_options
        )
        assert result == expected
    ...
    def test_preserve_original_case_enabled(
        ...
        options = TSVConversionOptions(case_insensitive=True, preserve_original_case=True)
    ...
    def test_preserve_original_case_disabled(
        ...
        options = TSVConversionOptions(case_insensitive=True, preserve_original_case=False)
        result = strategy.convert_text(input_text, default_conversion_dict, options)
        assert result == expected

    def test_mixed_content_conversion(
        self, strategy: CaseInsensitiveConversionStrategy, default_conversion_dict: dict[str, str]
    ) -> None:
...
class TestTSVConversionStrategyFactory:
    ...
    @pytest.mark.parametrize(
        "case_insensitive,expected_type",
        [
            (False, CaseSensitiveConversionStrategy),
            (True, CaseInsensitiveConversionStrategy),
        ],
    )
    def test_create_strategy(self, case_insensitive: bool, expected_type: type) -> None:
    ...
    def test_get_available_strategies(self) -> None:
        ...
        expected = ["CaseSensitiveConversionStrategy", "CaseInsensitiveConversionStrategy"]
