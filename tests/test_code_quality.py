...
class TestCodeQuality:
    ...
    def test_coverage_configuration(self) -> None:
        ...
        required_coverage_sections = ["[tool.coverage.run]", "[tool.coverage.report]"]

        for section in required_coverage_sections:
            if section not in content:
                pytest.fail(
                    f"Coverage configuration section {section} missing from pyproject.toml"
                )
    ...
    def test_github_actions_workflow_integrity(self, workflow_files: list[Path]) -> None:
        ...
        critical_commands = ["uv sync --all-extras --dev --locked", "uv run pytest"]
