...
from string_multitool.exceptions import ValidationError, TransformationError, ConfigurationError
...
from string_multitool.models.types import TextSource, SessionState
...
class TestModernApplicationInterface:
    ...
    @pytest.mark.parametrize("invalid_dependency", [
        "config_manager",
    ...
    def test_initialization_with_invalid_dependencies(
        ...
        # Replace one dependency with None
        dependencies[invalid_dependency] = None
        
        with pytest.raises(ValidationError, match="Invalid dependency injection"):
            ApplicationInterface(**dependencies)
    
    @pytest.mark.parametrize("rule,expected_pattern", [
        ("/t", r".*trim.*"),
        ("/l", r".*lowercase.*"),
        ("/u", r".*uppercase.*"),
        ("help", r".*help.*")
...
class TestModernIOManager:
    ...
    @pytest.mark.parametrize("stdin_content", [
        "piped content",
        "multiline\ncontent\nhere",
        "unicode: ñáéíóú",
    ])
...
class TestModernInteractiveSession:
    ...
    def interactive_session(
        ...
        """Create InteractiveSession for testing."""
    ...
    def test_text_refresh(self, interactive_session: InteractiveSession, mock_clipboard: Mock):
        ...
        interactive_session.refresh_text()
    ...
    def test_session_state_management(self, interactive_session: InteractiveSession):
        ...
        state = interactive_session.get_session_state()
...
class TestModernCommandProcessor:
    ...
    @pytest.mark.parametrize("command,is_valid", [
        ("help", True),
        ("refresh", True),
        ("quit", True), 
        ("exit", True),
...
class TestModernSystemIntegration:
    ...
    def test_end_to_end_transformation_workflow(
        ...
        input_text = io_manager.get_input_text()
        result = transformation_engine.apply_transformations(input_text, "/t/l")
...
class TestPerformanceCharacteristics:
    ...
    def test_bulk_transformation_performance(
        ...
        start_time = time.perf_counter()
        results = [
            transformation_engine.apply_transformations(text, "/t/l") 
