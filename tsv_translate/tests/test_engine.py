...
class TestTSVTranslateEngine:
    ...
    def test_engine_context_manager(self, test_config):
        ...
        engine = TSVTranslateEngine(test_config)
    ...
    def test_complete_workflow(self, translate_engine, sample_tsv_file):
        """Test complete workflow from sync to conversion."""
        # 1. Sync TSV file
        sync_result = translate_engine.sync_file(sample_tsv_file)
        assert sync_result.is_successful
        
        # 2. List rule sets
        rule_sets = translate_engine.list_rule_sets()
        assert "test_rules" in rule_sets
        
        # 3. Convert text
        conversion_result = translate_engine.convert_text("hello", "test_rules")
        assert conversion_result.is_successful
        assert conversion_result.converted_text == "こんにちは"
        
        # 4. Get rule set info
        info = translate_engine.get_rule_set_info("test_rules")
        assert info is not None
        assert info["rule_count"] == 3
        
        # 5. Remove rule set
        remove_result = translate_engine.remove_rule_set("test_rules")
        assert remove_result.is_successful
    
    def test_directory_sync_workflow(self, translate_engine, temp_directory):
        ...
        # Sync directory
        results = translate_engine.sync_directory(temp_directory)
        ...
        for i in range(3):
            result = translate_engine.convert_text("hello", f"rules_{i}")
            assert result.is_successful
            assert result.converted_text == "こんにちは"
    
    def test_error_handling_invalid_file(self, translate_engine, temp_directory):
        ...
        result = translate_engine.sync_file(invalid_file)
        assert not result.is_successful
        assert "Expected 2 columns" in result.error_message
    
    def test_error_handling_missing_rule_set(self, translate_engine):
        """Test error handling with missing rule set."""
        result = translate_engine.convert_text("test", "nonexistent")
        assert not result.is_successful
    ...
    def test_uninitialized_engine_error(self, test_config):
        ...
        with pytest.raises(TSVTranslateError, match="not initialized"):
    ...
    def test_concurrent_operations(self, translate_engine, temp_directory):
        ...
        results = []
        for file in files:
            result = translate_engine.sync_file(file)
        ...
        for i in range(5):
            result = translate_engine.convert_text(f"key_{i}", f"concurrent_{i}")
            assert result.is_successful
            assert result.converted_text == f"value_{i}"
    
    def test_performance_large_rule_set(self, translate_engine, temp_directory):
        ...
        result = translate_engine.sync_file(large_file)
        assert result.is_successful
        assert result.rules_processed == 1000
        
        # Conversion should be fast
        conversion_result = translate_engine.convert_text("key_0500", "large_rules")
        assert conversion_result.is_successful
