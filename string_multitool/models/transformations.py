...
# TSVTransformationクラスは tsv_transformer.py に移動されました
...
class TextTransformationEngine(ConfigurableComponent[dict[str, Any]], TransformationBase):
    ...
    def __init__(self, config_manager: ConfigManagerProtocol) -> None:
        ...
        if config_manager is None:
            raise ValidationError(
                "Configuration manager cannot be None", {"parameter": "config_manager"}
            )
        ...
        try:
            ...
            try:
                ...
                raise TransformationError(
                    ...
                    {
                        "config_keys": (
                            list(transformation_config.keys())
    ...
    def apply_transformations(self, text: str, rule_string: str) -> str:
        ...
        try:
            ...
            if not isinstance(rule_string, str):
                raise ValidationError(
                    VALIDATION_CONSTANTS.INVALID_RULE_TYPE_MSG.format(
                        type_name=type(rule_string).__name__
                    ),
            ...
            if not rule_string.startswith(VALIDATION_CONSTANTS.RULE_PREFIX):
                raise ValidationError(
                    VALIDATION_CONSTANTS.RULE_PREFIX_MSG,
        ...
        try:
            ...
            raise ValidationError(
                ...
                (
                    ...
                    else {ERROR_CONTEXT_KEYS.RULE_STRING: rule_string}
                ),
            ) from e
        except Exception as e:
            raise ValidationError(
        ...
        rules.update(
            {
                ...
                "tsvtr": TransformationRule(
                    name="Tsv Transform",
                    description="Convert text using TSV file rules. Use SQLite database for rule management",
    ...
    def _apply_tsv_conversion_simple(self, text: str, args: list[str]) -> str:
        """TSVファイルを使用した変換を実行
        ...
        try:
            ...
            # 新しいTSVTransformerを使用して変換実行
            from .tsv_transformer import TSVTransformer

            tsv_transformer = TSVTransformer(str(tsv_file_path), options)
    ...
    def _parse_tsv_conversion_args(self, args: list[str]) -> dict[str, Any]:
        ...
        try:
            if not args:
                raise ValidationError("TSV file path is required", {"args_provided": 0})
            ...
            return {"file_path": file_path, "options": options}
