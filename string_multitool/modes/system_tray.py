...
class SystemTrayMode:
        ...
        try:
            ...
            menu = pystray.Menu(
                ...
                pystray.MenuItem(
                    ...
                    checked=lambda _: self.daemon_mode.is_running if self.daemon_mode else False
                ),
                pystray.MenuItem(
                    "Stop Monitoring", 
                ...
                pystray.Menu.SEPARATOR,
                pystray.MenuItem("Set Preset 1", lambda: self._set_preset("1")),
                pystray.MenuItem("Set Preset 2", lambda: self._set_preset("2")),
                pystray.MenuItem("Set Preset 3", lambda: self._set_preset("3")),
                pystray.Menu.SEPARATOR,
        ...
        try:
            ...
            raise StringMultitoolError(
                f"Failed to start monitoring from system tray: {e}",
                {
                    ...
                    "daemon_running": self.daemon_mode.is_running if self.daemon_mode else False
                }
            ) from e
    ...
    def _show_status(self, icon: Any, item: Any) -> None:
        ...
        try:
            if self.daemon_mode:
                ...
                self._logger.info(f"[TRAY] Status: {'Running' if status['running'] else 'Stopped'}")
                self._logger.info(f"[TRAY] Active rules: {' -> '.join(status['active_rules']) if status['active_rules'] else 'None'}")
                self._logger.info(f"[TRAY] Active preset: {status['active_preset'] or 'None'}")
        except Exception as e:
            raise StringMultitoolError(
    ...
    def _show_help(self, icon: Any, item: Any) -> None:
        ...
        self._logger.info("[TRAY] - Start/Stop Monitoring: Control clipboard monitoring")
    ...
    def run(self) -> None:
        ...
        try:
            if not self.icon:
                raise StringMultitoolError(
                    "System tray icon not initialized",
                    {"component": "system_tray"}
                )
