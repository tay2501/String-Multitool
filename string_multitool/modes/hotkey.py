...
class HotkeyMode:
    ...
    def __init__(
        ...
        # Initialize sequence state manager
        timeout = self._config.get("hotkey_settings", {}).get(
    ...
    def _on_sequence_key(self, key: str) -> None:
        ...
        try:
            if self.sequence_state.is_sequence_active():
                ...
                self.logger.info(
                    f"Executing sequence command: {first_key}→{key} = {command}"
                )
                self._execute_command(command)
                self.sequence_state.end_sequence()
            else:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            for hotkey_str, command in self._direct_hotkeys.items():
                try:
                    keyboard.add_hotkey(
                        hotkey_str, lambda cmd=command: self._on_direct_hotkey(cmd)
                    )
                    self._registered_hotkeys.append(hotkey_str)
                except Exception as e:
                    self.logger.warning(
                        f"Failed to register direct hotkey {hotkey_str}: {e}"
                    )

            # Register sequence hotkeys with unified handler
            modifier = self._config.get("hotkey_settings", {}).get(
            ...
            for key in all_sequence_keys:
                ...
                try:
                    keyboard.add_hotkey(
                        sequence_hotkey, lambda k=key: self._on_sequence_key(k)
                    )
                    self._registered_hotkeys.append(sequence_hotkey)
                except Exception as e:
                    self.logger.warning(
                        f"Failed to register sequence hotkey {sequence_hotkey}: {e}"
                    )
            ...
            self.logger.info(
                f"Sequence hotkeys: {len(self._sequence_hotkeys)} registered"
            )
