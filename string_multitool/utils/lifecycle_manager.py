...
class ApplicationLifecycleManager:
    ...
    def _log_application_start(self) -> None:
        ...
        try:
            ...
            self._logger.info(f"APPLICATION_LIFECYCLE: {json.dumps(startup_log, ensure_ascii=False)}")
            ...
            self._logger.debug(f"System: {system_info['platform']} {system_info['platform_version']}")
    ...
    def unregister_observer(self, observer: LifecycleObserver) -> None:
        ...
        try:
            ...
            self._logger.warning(f"Observer not found for unregistration: {type(observer).__name__}")
    ...
    def log_application_end(self, exit_reason: ExitReason, exit_code: int = 0, exception: Optional[Exception] = None) -> None:
        """
        ...
        try:
            ...
            if exception is not None:
                debug_info["error_details"] = {
                    ...
                    "line_number": traceback.extract_tb(exception.__traceback__)[-1].lineno if exception.__traceback__ else 0
                }
            ...
            # Log structured data for machine processing
            self._logger.info(f"APPLICATION_LIFECYCLE: {json.dumps(shutdown_log, ensure_ascii=False)}")
            
            # Log human-readable shutdown message
            duration = session_info["duration_seconds"] or 0
            self._logger.info(f"Application ended - Duration: {duration:.3f}s, Reason: {exit_reason.value}")
            self._logger.debug(f"End timestamp: {session_info['end_timestamp_iso']}")
            self._logger.debug(f"Exit code: {exit_code}")
            self._logger.debug(f"Peak memory usage: {debug_info['resources']['memory_peak_mb']:.1f} MB")
            
            if exception:
                self._logger.error(f"Exception caused exit: {type(exception).__name__}: {str(exception)}")
...
def log_application_end(exit_reason: ExitReason, exit_code: int = 0, exception: Optional[Exception] = None) -> None:
    """
