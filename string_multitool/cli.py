...
def transform_text(
    rules: Annotated[str, typer.Argument(help="Transformation rules (e.g., '/t/l' for trim + lowercase)")],
    text: Annotated[
        Optional[str], 
    ...
    output: Annotated[
        bool, 
        typer.Option("--output", "-o", help="Copy result to clipboard")
    ] = True,
) -> None:
...
def daemon_mode(
    rules: Annotated[
        Optional[str], 
        typer.Option("--rules", "-r", help="Transformation rules to apply automatically")
    ] = None,
    preset: Annotated[
        Optional[str], 
        typer.Option("--preset", "-p", help="Use predefined rule preset")
    ] = None,
) -> None:
...
def show_rules(
    category: Annotated[
        Optional[str], 
        typer.Option("--category", "-c", help="Filter by category (basic, case, string, advanced)")
    ] = None,
    search: Annotated[
        Optional[str], 
        typer.Option("--search", "-s", help="Search rules by keyword")
    ] = None,
) -> None:
    ...
    try:
        ...
        for rule_key, rule_info in rules.items():
            # Apply filters
            if category and hasattr(rule_info, 'rule_type') and rule_info.rule_type.name.lower() != category.lower():
                continue
        ...
        console.print("  [cyan]echo 'text' | string-multitool transform '/p'[/cyan] - PascalCase from pipe")
